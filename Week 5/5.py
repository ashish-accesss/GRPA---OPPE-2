...
def groupby(data:list, key:callable):
    ...
    grouped_data ={}
    for item in data:
        grouped_key = key(item)
        if grouped_key not in grouped_data:
            grouped_data[grouped_key]=[]
...
def sort_rollno_by_marks(student_data, course1, course2, course3):
    ...
    '''
    return list(map(lambda student: student['rollno'],sorted(student_data,key=lambda x: (x[course1],x[course2],x[course3],x['rollno'])
    )))
